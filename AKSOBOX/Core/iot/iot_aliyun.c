...
void MQTT_Init(uint8_t *prx,uint16_t rxlen,uint8_t *ptx,uint16_t txlen)
{
	...
	memset(esp8266_buf,0,sizeof(esp8266_buf)); //清空接收缓冲
...
uint8_t MQTT_Connect(char *ClientID,char *Username,char *Password)
{
  ...
  esp8266_txbuf[MQTT_TxLen++] = 0x10;		//MQTT Message Type CONNECT
	...
	while(cnt--)
	{
		...
		{
			//CONNECT
			if(esp8266_buf[0]==parket_connetAck[0] && esp8266_buf[1]==parket_connetAck[1]) //连接成功			   
			{
				printf("connected");
				return 1;//连接成功
			}
...
{    
	...
	while(cnt--)
	{
		...
		while(wait--)
		{
			if(esp8266_buf[0]==parket_subAck[0] && esp8266_buf[1]==parket_subAck[1]) //订阅成功			   
			{
				return 1;//订阅成功
			}
			HAL_Delay(100);			
